...
#define DRUM_NO_LOOP 0
#define KEYBOARD_NO_LOOP 1
#define DRUM_LOOP 2 
#define KEYBOARD_LOOP 3
...
static const uint8_t ledLUT[16] = {0x3A, 0x37, 0x35, 0x34,
    0x28, 0x29, 0x23, 0x24,
    0x16, 0x1B, 0x11, 0x10,
    0x0E, 0x0D, 0x0C, 0x02},
buttonLUT[16] = {0x07, 0x04, 0x02, 0x22,
    0x05, 0x06, 0x00, 0x01,
    0x03, 0x10, 0x30, 0x21,
    0x13, 0x12, 0x11, 0x31};
...
static const uint8_t show7_2[7][24] = {
...
static const uint8_t i2xy64[] = {// Remap 8x8 TrellisSet button index to column/row
...
static const uint8_t drum_midi[] ={
...
void Myusartinit();
//midi music func
void send_note(char one, char two, char three);
//void noteOff(char channel, char pitch, char velocity);
//animation show func
...
void main(void) {
    ...
    uint8_t column = 0, rightbutton = 0, m = 0, n = 0, h = 0, k = 0;
        ...
        if (read_switch()) {
            for (h = 0; h < 64; h++) {
                ...
                if (is_key_pressed(h)&&(!was_key_pressed(h))) {
                    if (press[h] == 0) {
                        if (play_mode == DRUM_NO_LOOP || play_mode == DRUM_LOOP) {
                            send_note(0x99, drum_midi[h], 0x40);
                        } else if (play_mode == KEYBOARD_NO_LOOP || play_mode == KEYBOARD_LOOP) {
                            send_note(0x90, note, 127);
                        }
                        press[h] = 1;
                    } else {
                        if (play_mode == DRUM_NO_LOOP) {
                            send_note(0x99, drum_midi[h], 0x40);
                        } else if (play_mode == KEYBOARD_NO_LOOP) {
                            send_note(0x90, note, 127);
                        } else if (play_mode == DRUM_LOOP) {
                            send_note(0x89, drum_midi[h], 0x00);
                        } else if (play_mode == KEYBOARD_LOOP) {
                            send_note(0x80, note, 127);
                        }
                        press[h] = 0;
                    }
                    for (i = 0; i < 8; i++) { //right most button
                        if (h == Buttons[i][7]) {
                            rightbutton = (h - 19) >> 2; //0.1.2.3.8.9.10.11                            
                        }else{
                            press[Buttons[i][7]] = 0;
                        }
                    }
                    ...
                    if (rightbutton == 0 && column != 19) { //animation mode 1
                        ...
                        m = h >> 4; //button is in which matrix
                        n = h & 0x0f;
                        animation1(m, n);
...
void send_note(char one, char two, char three) {
    while (!PIR1bits.TXIF || !TRMT);
    TXREG = one; //noteOff.header
    while (!PIR1bits.TXIF || !TRMT);
    TXREG = two; //noteOff.byte2;
    while (!PIR1bits.TXIF || !TRMT);
    TXREG = three; //noteOff.byte3;
}
